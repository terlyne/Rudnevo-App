...
async def apply_for_vacancy(
    *,
    ...
    resume_link: Optional[str] = Form(None, description="Ссылка на резюме (опционально)"),
    vacancy_id: int = Form(..., description="ID вакансии"),
    resume_file: Optional[UploadFile] = File(None, description="Файл резюме (опционально)"),
):
...
async def bulk_update_student_status(
    *,
    ...
    updated_count = await student_crud.bulk_update_status(
        db=db,
        student_ids=bulk_update.student_ids,
