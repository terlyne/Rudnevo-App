...
def home():
    if request.method == "GET":
        ...
        return render_template("panel/home.html", nav_elements=nav_elements, last_actions=last_actions)
        ...
        for feedback in feedbacks:
            if 'created_at' in feedback:
                try:
                    ...
                    feedback['created_at'] = datetime.now()  # или другое значение по умолчанию
        
        return render_template("panel/feedback/feedback_list.html", 
...
def news_list():
    if request.method == "GET":
        ...
        return render_template("panel/news/news_list.html", nav_elements=nav_elements, news=news)
    ...
...
def reviews_list():
    if request.method == "GET":
        ...
        return render_template("panel/reviews/reviews_list.html", nav_elements=nav_elements, reviews=reviews)
    ...
...
def schedule_list():
    if request.method == "GET":
        ...
        return render_template("panel/schedule/schedule_list.html", nav_elements=nav_elements, schedule=schedule)
    ...
...
def users_list():
    if request.method == "GET":
        ...
        return render_template("panel/users/users_list.html", nav_elements=nav_elements, users=users)
    ...
...
def vacancies_list():
    if request.method == "GET":
        ...
        return render_template("panel/vacancies/vacancies_list.html", 
                             nav_elements=nav_elements, 
                             vacancies=vacancies,
...
def vacancy_create():
    if request.method == "GET":
        nav_elements = get_navigation_elements()
        return render_template("panel/vacancies/vacancy_form.html", 
                             nav_elements=nav_elements, 
                             vacancy=None,
                             is_edit=False)
    ...
    if request.method == "POST":
        try:
            ...
            if start_date and end_date:
                ...
                if start_dt >= end_dt:
                    flash("Дата начала должна быть раньше даты окончания", category="error")
...
@panel.route("/vacancies/<int:vacancy_id>/edit", methods=["GET", "POST"], endpoint="vacancy_edit")
@login_required
...
@panel.route("/vacancies/<int:vacancy_id>/delete", methods=["POST"], endpoint="vacancy_delete")
@login_required
...
@panel.route("/vacancies/<int:vacancy_id>/applications", methods=["GET", "POST"], endpoint="vacancy_applications")
@login_required
def vacancy_applications(vacancy_id):
    if request.method == "GET":
        ...
        return render_template("panel/vacancies/vacancy_applications.html", 
                             nav_elements=nav_elements, 
                             vacancy=vacancy,
        ...
        try:
            ...
            if action == 'bulk_update_status':
                ...
                if student_ids and status:
                    ...
                    flash(f"Статус {len(student_ids)} заявок обновлен", category="success")
...
def profile():
    if request.method == "GET":
        ...
        return render_template("panel/profile.html", nav_elements=nav_elements, user=current_user)
    ...
